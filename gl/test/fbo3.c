...
void display()
{
	...
	glColor3f(1, 0, 0);
	gl_draw_quads(0, 32, 0, 16, 16);
	glColor3f(0, 1, 0);
	gl_draw_quads(0, 16, 0, 16, 16);
	glColor3f(0, 0, 1);
	gl_draw_quads(16, 32, 0, 16, 16);
	glColor3f(1, 0, 1);
	gl_draw_quads(16, 16, 0, 16, 16);
	glColor3f(0.4, 0.4, 0);
	gl_draw_quads(8, 24, 0, 16, 16);
	...
	glLoadIdentity();
	glBindTexture(GL_TEXTURE_2D, fbotex[0]);
	gl_draw_quads(-0.4, 0.4, 0.2, 0.2, 0.2);

	static char Y[32*32], U[16*16], V[16*16];
	void *data2[3] = {Y, U, V};
	int line2[3] = {32, 16, 16};

	glBindTexture(GL_TEXTURE_2D, fbotex[1]);
	gl_draw_quads(0.0, 0.4, 0.2, 0.2, 0.2);
	glGetTexImage(GL_TEXTURE_2D, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, Y);
	dump("/tmp/Y", Y, 32*32);

	glBindTexture(GL_TEXTURE_2D, fbotex[2]);
	gl_draw_quads(-0.4, 0.0, 0.2, 0.2, 0.2);
	glGetTexImage(GL_TEXTURE_2D, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, U);

	glBindTexture(GL_TEXTURE_2D, fbotex[3]);
	gl_draw_quads(0.0, 0.0, 0.2, 0.2, 0.2);
	glGetTexImage(GL_TEXTURE_2D, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, V);
